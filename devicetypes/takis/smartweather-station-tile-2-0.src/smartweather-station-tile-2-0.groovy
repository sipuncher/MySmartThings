...
metadata {
	definition (name: "SmartWeather Station Tile 2.0", namespace: "takis", author: "Takis") {
        ...
        attribute "precipTodayMM", "number"
        attribute "precipLastHrMM", "number"
...
def poll() {
	...
	if (obs) {
        log.debug "obs --> ${obs}"
        def now = new Date().format('h:mm:ss aa d/MM/yyyy',location.timeZone)
        ...
        if (height_units) {
                switch (height_units) {
                    case "height_in" :
                        ...
                        send(name: "precipTodayMM", value: "${obs.precip_today_metric}")
                        send(name: "precipLastHrMM", value: "${obs.precip_1hr_metric}")
                    break;
		...
		def cityValue = "${obs.display_location.city}"
